...
namespace Cbeua.Api.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class AccountDirecyEntryController : ControllerBase
    {
        private readonly IAccountDirectEntryService _service;
        public AccountDirecyEntryController(IAccountDirectEntryService service)
        {
            _service = service;
        }
        [HttpGet]
        public async Task<CustomApiResponse> GetAll()
        {
            ...
            try
            {
                var accountsDirectEntries = await _service.GetAllAsync();
                response.IsSucess = true;
                response.Value = accountsDirectEntries;
        ...
        {
            var response = new CustomApiResponse();
            var accountsDirectEntry = await _service.GetByIdAsync(id);
        ...
        public async Task<CustomApiResponse> Create([FromBody] AccountsDirectEntry accountsDirectEntry)
        {
            ...
            try
            {
                var created = await _service.CreateAsync(accountsDirectEntry);
                response.IsSucess = true;
        ...
        public async Task<CustomApiResponse> Update(int id, [FromBody] AccountsDirectEntry accountsDirectEntry)
        {
            ...
            var updated = await _service.UpdateAsync(accountsDirectEntry);
            if (!updated)
            {
                response.IsSucess = false;
                response.Error = "Not found";
