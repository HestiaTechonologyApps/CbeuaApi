...
namespace Cbeua.Api.Controllers
{
    ...
    {
        private readonly IDesignationsService _service;
        public DesignationController(IDesignationsService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        public async Task<CustomApiResponse> Create([FromBody] Designation designation)
        {
            ...
            try
            {
                var created = await _service.CreateAsync(designation);
                response.IsSucess = true;
