...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    //[Authorize]
    public class HRMInterviewTypeController : ControllerBase
    {
        private readonly IHRMInterviewTypeService _service;
        public HRMInterviewTypeController(IHRMInterviewTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMInterviewTypeCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMInterviewTypeCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMInterviewTypePaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
