...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    [Authorize]
    public class HRMEmployeeAwardController : ControllerBase
    {
        private readonly IHRMEmployeeAwardService _service;

        public HRMEmployeeAwardController(IHRMEmployeeAwardService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMEmployeeAwardCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMEmployeeAwardCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMEmployeeAwardPaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
