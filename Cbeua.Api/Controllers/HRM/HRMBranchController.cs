...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    [Authorize]
    public class HRMBranchController : ControllerBase
    {
        private readonly IHRMBranchService _service;

        public HRMBranchController(IHRMBranchService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMBranchCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMBranchCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMBranchPaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
