...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    {
        private readonly IHRMSJobTypeService _service;

        public HRMJobTypesController(IHRMSJobTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMSJobTypeCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMSJobTypeCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMSJobTypePaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
