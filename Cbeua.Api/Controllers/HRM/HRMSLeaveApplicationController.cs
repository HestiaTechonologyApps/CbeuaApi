...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    [Authorize]
    public class HRMSLeaveApplicationController : ControllerBase
    {
        private readonly IHRMSLeaveApplicationService _service;

        public HRMSLeaveApplicationController(IHRMSLeaveApplicationService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMSLeaveApplicationCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMSLeaveApplicationCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMSLeaveApplicationPaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
