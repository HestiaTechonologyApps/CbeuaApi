...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
   // [Authorize]
    public class HRMJobLocationController : ControllerBase
    {
        private readonly IHRMJobLocationService _service;
        public HRMJobLocationController(IHRMJobLocationService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMJobLocationCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMJobLocationCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMJobLocationPaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
