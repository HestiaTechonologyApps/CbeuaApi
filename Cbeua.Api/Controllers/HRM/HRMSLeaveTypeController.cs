...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    [Authorize]
    public class HRMSLeaveTypeController : ControllerBase
    {
        private readonly IHRMSLeaveTypeService _service;

        public HRMSLeaveTypeController(IHRMSLeaveTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMSLeaveTypeCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMSLeaveTypeCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMSLeaveTypePaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
