...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    [Authorize]
    public class HRMDocumentTypeController : ControllerBase
    {
        private readonly IHRMDocumentTypeService _service;

        public HRMDocumentTypeController(IHRMDocumentTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMDocumentTypeCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMDocumentTypeCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMDocumentTypePaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
