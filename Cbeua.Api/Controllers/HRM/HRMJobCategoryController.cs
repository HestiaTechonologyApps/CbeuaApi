...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
   // [Authorize]
    public class HRMJobCategoryController : ControllerBase
    {
        private readonly IHRMJobCategoryService _service;
        public HRMJobCategoryController(IHRMJobCategoryService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMJobCategoryCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMJobCategoryCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMJobCategoryPaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
