...
namespace Cbeua.Api.Controllers.HRMS
{
    ...
    [ApiController]
    //[Authorize]
    public class HRMCandidateController : ControllerBase
    {
        private readonly IHRMCandidateService _service;

        public HRMCandidateController(IHRMCandidateService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] HRMCandidateCreateUpdateDTO entitydto)
        {
            return await _service.CreateAsync(entitydto);
        }

        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] HRMCandidateCreateUpdateDTO entitydto)
        {
            try
            {
                if (id != entitydto.Id)
        ...
        public async Task<CustomApiResponse> GetPaged([FromBody] HRMCandidatePaginationParams parameters)
        {
            var response = new CustomApiResponse();
            try
            {
